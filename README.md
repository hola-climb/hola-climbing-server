...
| Build | Maven |
| Test | JUnit 5, Testcontainers (pgvector PostgreSQL · Redis) |
...
별도 `psql -f db/schema.sql` 명령은 필요하지 않습니다.
서버가 처음 기동될 때 Flyway가 `src/main/resources/db/migration/V1__init.sql`을 자동 적용합니다.
`db/schema.sql`은 전체 스키마를 한눈에 보기 위한 snapshot으로 유지합니다.

이미 `db/schema.sql` 또는 `db/manual-migrations/*`로 생성한 DB는 바로 Flyway V1을 실행하면
테이블 중복으로 실패할 수 있습니다. 운영 반영 전 백업을 만든 뒤 1회에 한해
...
| `SPRING_FLYWAY_ENABLED` | Flyway migration 활성화 여부 | `true` |
| `SPRING_FLYWAY_BASELINE_ON_MIGRATE` | 기존 DB 1회 baseline 여부 | `false` |
...
| `FRONTEND_BASE_URL` | 메일 인증·공유 링크용 단일 프론트 URL | `http://localhost:5173` |
...
| `CORS_ALLOWED_ORIGINS` | 허용 Origin 목록 (쉼표 구분 가능) | `http://localhost:5173` |
| `OAUTH_ALLOWED_REDIRECT_URIS` | OAuth 완료 후 프론트로 돌려보낼 허용 URI 목록 | `http://localhost:5173/oauth/callback` |
| `GOOGLE_OAUTH_CLIENT_ID` / `GOOGLE_OAUTH_CLIENT_SECRET` | Google OAuth client credentials | (없음) |
| `KAKAO_OAUTH_CLIENT_ID` / `KAKAO_OAUTH_CLIENT_SECRET` | Kakao OAuth REST API credentials | (없음) |
| `NAVER_OAUTH_CLIENT_ID` / `NAVER_OAUTH_CLIENT_SECRET` | Naver OAuth client credentials | (없음) |
| `MONTHLY_REPORT_MIN_VIDEOS` / `MONTHLY_REPORT_MIN_PROBLEMS` | 월간 리포트 콜드스타트 기준 | `3` / `10` |
| `MONTHLY_REPORT_SCHEDULER_ENABLED` / `MONTHLY_REPORT_SCHEDULER_CRON` | 월간 리포트 사전 생성 스케줄러 활성화와 cron | `false` / `0 0 0 1 * *` |
...
1. 프론트는 `GET /api/auth/oauth/{provider}/authorize?redirectUri={frontendCallback}`로 이동합니다.
2. 백엔드는 provider 인증 후 `{frontendCallback}?oauthCode=...`로 돌려보냅니다.
3. 프론트는 `POST /api/auth/oauth/result`에 `{ "code": "{oauthCode}" }`를 보내고 `data.status`로 분기합니다.

| `data.status` | 의미 | 주요 필드 |
|---|---|---|
| `LOGGED_IN` | 기존 소셜 계정 로그인 완료 | `token` |
| `SIGNUP_REQUIRED` | 신규 소셜 회원가입 필요 | `signupToken`, `email`, `suggestedNickname`, `profileImage` |
| `EMAIL_ALREADY_EXISTS` | 같은 이메일의 로컬 계정이 이미 존재 | `email`, `suggestedNickname`, `profileImage` |

`EMAIL_ALREADY_EXISTS`는 자동 계정 연결을 하지 않으며 `token`과 `signupToken`을 내려주지 않습니다. 프론트는 기존 이메일 로그인 또는 별도 계정 연결 흐름을 안내해야 합니다.
...
# 또는
./mvnw clean package && java -jar target/server-*.jar
```
...
- Fresh DB는 앱 기동 시 Flyway가 `V1__init.sql`부터 자동 적용합니다.
  기존 운영 DB는 백업 후 1회 baseline 처리하고, 이후 변경은 새 `V{n}__*.sql` migration으로 반영합니다.
- `JWT_SECRET`, `SPRING_DATASOURCE_*`, `SPRING_DATA_REDIS_*`, `GCS_BUCKET`,
  GCS 자격증명, `FRONTEND_BASE_URL`, `CORS_ALLOWED_ORIGINS`, `AI_CALLBACK_SECRET`,
  `APP_MAIL_MODE=smtp`, SMTP 호스트/계정, `*_OAUTH_CLIENT_ID`, `*_OAUTH_CLIENT_SECRET`은 운영 값으로 분리합니다.
  `FRONTEND_BASE_URL`은 메일/공유 링크에 들어갈 단일 URL이고,
  `CORS_ALLOWED_ORIGINS`는 쉼표로 구분한 허용 Origin 목록입니다. `OAUTH_ALLOWED_REDIRECT_URIS`는
  OAuth 완료 후 redirect 가능한 프론트 callback URI만 쉼표로 등록합니다.
...
| 도메인 | 기능 |
|--------|------|
| 회원·인증 | 이메일 인증 회원가입, 로그인, 카카오·네이버·구글 OAuth 로그인, JWT 토큰 재발급, 로그아웃, 비밀번호 재설정, 회원 탈퇴 |
...
| 통계 | 내 클라이밍 통계, 기술별(동작별) 사용 빈도 통계, 월간 AI 리포트 |
| 암장 리뷰 | 리뷰 작성·목록·수정·삭제 (사용자당 암장 1개 리뷰), 평점 자동 재집계 |
| 암장 관리 | 사용자 암장 등록 제안(승인 대기), 관리자 전용 암장 프로필 이미지 업로드 / 교체 |
| 클라이밍 기록·달력 | "어느 암장에서 난이도별 몇 문제를 풀었다"는 기록 CRUD, 월간·일별 달력 조회 |
| 추천 | pgvector 유사도 + 팔로잉 boost 기반 개인화 피드, 근처 암장 추천, 특정 공개 영상의 최종 분석 기술·dynamic/static과 유사한 암장 추천 |
...
- **분석 결과 수신·조회** — AI 워커가 세그먼트 raw 결과와 영상 단위 대표 결과
  (7가지 동작 목록, dynamic/static 추론값, 확률)를 `X-AI-Callback-Secret` 헤더가 포함된 콜백으로
  전달하면 서버가 저장합니다. 클라이언트 조회는 영상 대표 결과 기준이며 dynamic/static은
  세그먼트 비율로 재계산하지 않습니다.
- **분석 피드백 / 재시도** — 영상 소유자의 피드백으로 최종 동작 목록과 dynamic/static 값을 보정하고,
  이 최종값을 모델 정확도 통계와 이후 재학습 데이터로 사용합니다. 실패 시 재시도 요청.
- **영상 기반 암장 추천** — `GET /api/videos/{videoId}/recommendations/gyms`는 공개 기준 영상의
  `analysis_video_results.final_techniques`, `final_is_dynamic`을 암장별 공개 분석 영상 분포와 비교합니다.
  분석 데이터가 부족하면 근처 암장 fallback으로 내려주며, 응답에는 `technique`, `dynamic`, `distance`, `rating`
  reason 키워드를 포함합니다.
...
├── db/
│   └── schema.sql                  # 전체 DB 스키마 snapshot (테이블 · 인덱스 · 초기 데이터)
